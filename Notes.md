...
The integration with Zephyr Cloud is very smooth and fast, everything is deployed instantly with zero config, I used to use ngrok and vercel for deploying and sharing a preview link with my collegues before but with Zephyr Cloud I don't need to worry about that anymore.

I tried to setup a ci/cd pipeline with github actions to run e2e tests to make sure everything is working before the deployment but I turns out I need to deploy the app to have its url. I had some issues with the configuration and I couldn't make it work in time, so I decided to focus on the development of the app and leave the ci/cd setup for later. The .github/workflows/playwright.yml file is commented.
